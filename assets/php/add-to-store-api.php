...
error_reporting(0);
//Database Connection...
$s_conn = mysqli_connect('localhost','outfitte_store','+F%JW[$YDOR(','outfitte_opencart') or die('Error: ' . $s_conn->error);
...
if(mysqli_num_rows($ag) <= 0){
  ...
  if(mysqli_num_rows($cg) > 0){
    ...
    $ig = mysqli_query($s_conn, $iq) or die('Insert oc_product error: ' . $s_conn->error);
    ...
    mysqli_query($s_conn, $diq) or die('Insert oc_product_description error: ' . $s_conn->error);
    ...
    foreach($is_array as $is){
        if($_REQUEST['product_'.$is] != ''){
          ...
          $ag = mysqli_query($s_conn, $aq) or die('Select oc_attribute_description error ' . $s_conn->error);
          if(mysqli_num_rows($ag) <= 0){
            //Get new Attribute ID...
            $aiq = "INSERT INTO `oc_attribute` (`attribute_group_id`,`sort_order`) VALUES ('6','0')";
            $aig = mysqli_query($s_conn, $aiq) or die('Insert oc_attribute error ' . $s_conn->error);
            ...
            $adig = mysqli_query($s_conn, $adiq) or die('Insert oc_attribute_description error ' . $s_conn->error);
          }else{
          ...
          mysqli_query($s_conn, $paiq) or die('Insert oc_product_attribute error: ' . $s_conn->error);
    ...
    $cuq = "INSERT INTO `oc_product_to_category` (`product_id`,`category_id`) VALUES ('" . $new_product_id . "','" . $product_81_store_category . "')";
    mysqli_query($s_conn, $cuq) or die($s_conn->error);
    $x->message .= ' - Category Inserted';
    ...
    if($product_size != ''){
      ...
      $fg = mysqli_query($s_conn, $fq) or die($s_conn->error);
      if(mysqli_num_rows($fg) > 0){
        ...
        mysqli_query($s_conn, $ifq) or die($s_conn->error);
        ...
        mysqli_query($s_conn, $ifdq) or die($s_conn->error);
      }
      $ipfq = "INSERT INTO `oc_product_filter` (`product_id`,`filter_id`) VALUES ('" . $new_product_id . "','" . $filter_id . "')";
      mysqli_query($s_conn, $ipfq) or die($s_conn->error);
      $x->message .= ' - Size product filter inserted';
      
      //Insert Filter to Category...
      $cfq = "SELECT * FROM `oc_category_filter` WHERE `category_id` = '" . $product_81_store_category . "' AND `filter_id` = '" . $filter_id . "'";
      $cfg = mysqli_query($s_conn, $cfq) or die($s_conn->error);
      if(mysqli_num_rows($cfg) <= 0){
        $cfiq = "INSERT INTO `oc_category_filter` (`category_id`,`filter_id`) VALUES ('" . $product_81_store_category . "','" . $filter_id . "')";
        mysqli_query($s_conn, $cfiq) or die($s_conn->error);
      }
    }
    ...
    mysqli_query($s_conn, $siq) or die($s_conn->error);
