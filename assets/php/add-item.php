...
if ($response->Ack !== 'Failure') {
    echo '<script>
            document.getElementById("lStatus").innerHTML += " <span style=\"color:green;\">Ebay Store: LISTED [Item#: ' . $response->ItemID . ']</span><br><br>";
...
if($_REQUEST['submit_to_store'] == 'on'){
  ...
  if($store_response->response == 'GOOD'){
     ...
     echo '<script>
            document.getElementById("lStatus").innerHTML += " <span style=\"color:green;\">81O Store: LISTED [Item#: ' . $store_response->product_id . ']</span><br><br>";
            document.getElementById("success_btns").style.display = "inline-block";
            document.getElementById("failed_btns").style.display = "none";
