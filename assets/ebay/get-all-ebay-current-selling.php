...
if($_REQUEST['env_mode'] != ''){
  $env_mode = $_REQUEST['env_mode'];
}
/**
 ...
 */
if($_REQUEST['oauth_code'] != ''){
  $token = $_REQUEST['oauth_code'];
}else{
  $token = $config[$env_mode]['oauthUserToken'];
}
...
/*if($_REQUEST['oauth_code'] != ''){
  $request->RequesterCredentials->eBayAuthToken = $_REQUEST['oauth_code'];
}else{
